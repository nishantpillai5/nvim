...
-- Under this config's own state dir, so each NVIM_APPNAME stays isolated.
-- Neovim creates 'undodir' on demand but not 'backupdir', which fails silently.
vim.o.undofile = true
...
-- The 1000000 maximum; lines are allocated as they arrive, so this is free.
...
-- No display server in the container, so OSC 52 sends the yank to the host's
-- terminal. Paste needs the terminal to answer the query, which many do not.
